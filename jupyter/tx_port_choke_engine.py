#!/usr/bin/env python3
"""TX Port Choke Check — helper used by the Jupyter notebook.

You do not need to edit this file.
Keep it in the SAME folder as TX_Port_Choke_Check.ipynb.

Libraries (install once in Jupyter or Command Prompt):
...
def fmt_day_mon_yy(value) -> str:
    """22-Sep-26 — this form works on Windows and Linux."""
    stamp = pd.Timestamp(value)
    return f"{stamp.day}-{stamp.strftime('%b-%y')}"


def parse_date_column(series: pd.Series) -> pd.Series:
    """Date may be a real date, an Excel serial number, or text."""
...
def parse_hour_column(series: pd.Series) -> pd.Series:
    """Time may be '08:00', a datetime, or an Excel day-fraction."""
    if pd.api.types.is_numeric_dtype(series):
        hours = (series.astype(float) * 24.0).round() % 24
        return hours.astype(int)
    text = series.astype(str)
    extracted = text.str.extract(r"(\d{1,2})", expand=False)
    return extracted.fillna("0").astype(int) % 24
...
def find_column(df: pd.DataFrame, aliases: tuple[str, ...]) -> str:
    """Find a column even if the heading has extra spaces or different case."""
    wanted = {_normalize_name(a) for a in aliases}
    for col in df.columns:
        if _normalize_name(col) in wanted:
            return col
    raise KeyError(
...
def find_source_file(folder: Path, source_name: str = "") -> Path:
    """Pick the KPI file in the folder. Skips reports this notebook writes."""
    folder = Path(folder)
    if source_name:
    ...
    skip_prefix = ("fege_choked", "tx_port_choke", "~$")
    allowed = {".csv", ".xls", ".xlsx", ".xlsm", ".xlsb"}
    found = []
    for path in folder.iterdir():
        ...
        if path.suffix.lower() not in allowed:
            continue
        if path.name.lower().startswith(skip_prefix):
            continue
        found.append(path)
    if not found:
        raise FileNotFoundError(
            f"No CSV / XLS / XLSX / XLSB file found in {folder}"
        )
    found.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    return found[0]


def load_kpi_file(path: Path) -> pd.DataFrame:
    """Read CSV, XLS, XLSX or binary Excel (XLSB)."""
    path = Path(path)
    suffix = path.suffix.lower()
    if suffix == ".csv":
        raw = pd.read_csv(path)  # built into pandas
    elif suffix == ".xlsb":
        raw = pd.read_excel(path, engine="pyxlsb")  # pip install pyxlsb
    elif suffix == ".xls":
        raw = pd.read_excel(path, engine="xlrd")  # pip install xlrd
    else:
        raw = pd.read_excel(path, engine="openpyxl")  # pip install openpyxl

    date_col = find_column(raw, ("Date", "DATE"))
    time_col = find_column(raw, ("Time", "TIME"))
    site_col = find_column(
        raw,
        ("eNodeB Name", "eNodeBName", "Site", "Site Name", "NE Name"),
    )
    rx_col = find_column(
        raw,
        (RX_COL, "RxMaxSpeed", "VS.FEGE.RxMaxSpeed", "RxMaxSpeed(bit/s)"),
    )
    tx_col = find_column(
        raw,
        (TXBW_COL, "TxTotalBW", "VS.FEGE.TxTotalBW", "Tx Total BW"),
    )
    out = raw.rename(
        columns={
            date_col: "Date",
...
def excel_serial_to_datetime(series: pd.Series) -> pd.Series:
    return parse_date_column(series)
...
def analyse(df: pd.DataFrame) -> tuple[pd.DataFrame, list[dict]]:
    ...
    work["Hour"] = parse_hour_column(work["Time"])
    ...
    last_day_label = fmt_day_mon_yy(period_end)
...
def print_summary(work: pd.DataFrame, records: list[dict]) -> None:
...
def run_report(folder, source_name: str = "", output_name: str = "TX_Port_Choke_Check.xlsx"):
    """Load a KPI file from folder, find choked sites, write the Excel report."""
    folder = Path(folder)
    source = find_source_file(folder, source_name)
    output = folder / output_name
    print(f"Reading: {source}")
    raw = load_kpi_file(source)
    work, records = analyse(raw)
    _require_references(records)
    _write_workbook(output, source.name, work, records)
    print_summary(work, records)
    print(f"Wrote {output}")
    return work, records, output
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="",
        help="KPI file path, or leave empty and pass --folder",
    )
    parser.add_argument(
        "--folder",
        default=".",
        help="Folder that holds the KPI file and will receive the report",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="TX_Port_Choke_Check.xlsx",
        help="Report workbook name (saved in the folder)",
    )
    args = parser.parse_args()
    folder = Path(args.folder)
    source_name = Path(args.source).name if args.source else ""
    if args.source and Path(args.source).is_file():
        folder = Path(args.source).parent
        source_name = Path(args.source).name
    run_report(folder, source_name, args.output)
